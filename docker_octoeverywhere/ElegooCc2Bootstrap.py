...
class ElegooCc2Bootstrap:
    ...
    def Bootstrap(logger:logging.Logger, config:Config) -> None:
        ...
        if accessCode is not None:
            ...
            config.SetStr(Config.SectionElegoo, Config.ElegooCc2AccessToken, accessCode)
        elif config.GetStr(Config.SectionElegoo, Config.ElegooCc2AccessToken, None) is None:
            logger.info("No Elegoo CC2 access code configured; using the factory default token.")
            config.SetStr(Config.SectionElegoo, Config.ElegooCc2AccessToken, "123456")
